...
currentWd = os.getcwd()
print "Current working directory is: " + currentWd
# Change directory to where all the DLLs live.
cbeccInstallRoot = "C:/Program Files/CBECC-Com 2013"  # docker version
#cbeccInstallRoot = "C:/Program Files (x86)/CBECC-Com 2013/" # windows version
cbeccUserRoot = "C:/users/root/My Documents/CBECC-Com 2013 Data"  # docker version
#cbeccUserRoot = "C:/Users/nlong/Documents/CBECC-Com 2013 Data/" # windows version

os.chdir(cbeccInstallRoot)
lib = cdll.LoadLibrary('BEMCmpMgr_os.dll')
...
rulesPath = (cbeccUserRoot + "/Rulesets/CEC 2013 Nonres/CEC 2013 NonRes BEMBase.bin").replace("/", "\\")
bemType = 0
log = None

print("Initializing Library")
print("Rules path is: " + rulesPath)

if not os.path.exists(rulesPath):
    print "Could not find rules path: " + rulesPath
    raise OSError

try:
    lib.InitBEMProcAndCmpMgrDLLs(rulesPath, bemType, log)
except:
...
# move the analysis file into the windows portion of the system ??

pszBEMBasePathFile = None
pszRulesetPathFile = (cbeccUserRoot + "/Rulesets/CEC 2013 NonRes.bin").replace("/", "\\")
#/root/.wine/drive_c/users/root/My Documents/CBECC-Com 2013 Data/Rulesets/CEC 2013 NonRes.bin
pszSimWeatherPath = (cbeccUserRoot + "/EPW/").replace("/", "\\")
#blank is current directory -- but should be the path where EPlus and T24DHW directories live
pszCompMgrDLLPath = (cbeccInstallRoot + "/").replace("/", "\\")
pszDHWWeatherPath = None
pszProcessingPath = (currentWd + "/test/").replace("/", "\\")
pszModelPathFile = (currentWd + "/test/0200006-OffSml-SG-BaseRun.cibd").replace("/", "\\")
pszLogPathFile = None  #(currentWd + "/test/junk.log")
...
bDisplayProgress = False  # this is the progress bar (requires vfb)
hWnd = None
...
result = lib.CMX_PerformAnalysis_CECNonRes(pszBEMBasePathFile, pszRulesetPathFile, pszSimWeatherPath, pszCompMgrDLLPath,
                                           pszDHWWeatherPath, pszProcessingPath, pszModelPathFile, pszLogPathFile,
                                           pszUIVersionString, bLoadModelFile,
                                           pszAnalysisOptionsCSV, pszErrorMessage, iErrorMsgLength, bDisplayProgress,
                                           hWnd, pszResultsSummary, iResultsSummaryLen)
...
print str(result) + " :: " + errorCodes[result]